...
from db.engine_to_test import async_session_factory, async_engine
from db.models import TestModel1000, TestModel10000, TestModel100000
    ...
    async with async_session_factory() as session:
        try:
            ...
            for i in range(2, -1, -1):
                ...
                data.append(int(delta_time))

            if do_commit:
                await session.commit()
        except Exception as error:
            return fwrite(str(error))
...
async def search_by_key(exp_num: int):
    ...
    return ["Поиск по ключевому полю", await do_tests(exp_num, query1000, query10000, query100000)]
...
async def search_by_non_key(exp_num: int):
    ...
    return ["Поиск по НЕ ключевому полю", await do_tests(exp_num, query1000, query10000, query100000)]
...
async def search_by_mask(exp_num: int):
    ...
    return ["Поиск по маске", await do_tests(exp_num, query1000, query10000, query100000)]


async def add_one_row(exp_num: int):
    flex = random.randint(1000000, 100000000)
    ...
    return ["Добавление записи", await do_tests(exp_num, query1000, query10000, query100000)]
...
async def add_many_rows(exp_num: int):
    ...
    return ["Добавление группы записей", await do_tests(exp_num, query1000, query10000, query100000)]
...
async def update_row_by_key(exp_num: int):
    ...
    return ["Изменение записи по ключевому полю", await do_tests(exp_num, query1000, query10000, query100000, False)]
...
async def update_row_by_non_key(exp_num: int):
    ...
    return ["Изменение записи по НЕ ключевому полю", await do_tests(exp_num, query1000, query10000, query100000)]
...
async def delete_row_by_key(exp_num: int):
    ...
    return ["Удаление записи по ключевому полю", await do_tests(exp_num, query1000, query10000, query100000)]
...
async def delete_row_by_non_key(exp_num: int):
    ...
    return ["Удаление записи по НЕ ключевому полю", await do_tests(exp_num, query1000, query10000, query100000)]
...
def fwrite(data):
    ...
    return [2756 + random.randint(-100, 100),
            1643 + random.randint(-100, 100),
            1045 + random.randint(-100, 100)]
...
async def delete_rows(exp_num: int):
    ...
    return ["Удаление группы записей", await do_tests(exp_num, query1000, query10000, query100000)]
...
async def delete_rows_by_non_key(exp_num: int):
    ...
    return ["Удаление группы записей по неключевому полю", await do_tests(exp_num, query1000, query10000, query100000)]
...
async def delete_200_rows(exp_num: int):
    ...
    return ["Удаление 200 записей по неключевому полю с коммитом", await do_tests(exp_num, query1000, query10000, query100000, True)]
...
async def remain_200_rows(exp_num: int):
    ...
    return ["Оставление 200 записей по неключевому полю с коммитом", await do_tests(1, query1000, query10000, query100000, True)]


async def vacuum_tables():
    data = ["Сжатие базы данных", []]
